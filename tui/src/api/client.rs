...
impl ApiClient {
    pub fn new(port: u16) -> Self {
        Self::from_base_url(format!("http://localhost:{}", port))
    }
    ...
    pub async fn query(
        ...
        let mut body =
            json!({"query": query, "top_k": top_k, "include_embeddings": include_embeddings});
    ...
    pub async fn session_turns(
        ...
        session_id: &str,
        limit: i32,
    ) -> Result<SessionTurnListResponse> {
        Ok(self
            .client
            .get(format!(
                "{}/api/sessions/{}/turns?limit={}",
    ...
    pub async fn feedback_stats(&self) -> Result<FeedbackStatsResponse> {
        Ok(self
            .client
            .get(format!("{}/api/feedback/stats", self.base_url))
            .send()
            .await?
            .json()
            .await?)
    ...
    pub async fn eval_runs(&self) -> Result<EvalRunListResponse> {
        Ok(self
            .client
            .get(format!("{}/api/eval/runs", self.base_url))
            .send()
            .await?
            .json()
            .await?)
    ...
    pub async fn kg_entities(&self, limit: i32, offset: i32) -> Result<KgEntityListResponse> {
        Ok(self
            .client
            .get(format!(
                "{}/api/knowledge/entities?limit={}&offset={}",
                self.base_url, limit, offset
            ))
            .send()
            .await?
    ...
    pub async fn kg_neighbors(&self, entity_id: &str) -> Result<KgNeighborhood> {
        Ok(self
            .client
            .get(format!(
                "{}/api/knowledge/entities/{}/neighbors",
    ...
    pub async fn document_chunks(
        ...
        limit: i32,
        offset: i32,
    ) -> Result<ChunkListResponse> {
        Ok(self
            .client
            .get(format!(
                "{}/api/documents/{}/chunks?limit={}&offset={}",
                self.base_url, doc_id, limit, offset
            ))
            .send()
            .await?
            .error_for_status()?
